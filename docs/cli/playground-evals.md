...
For a selected package, including dotted packages:

```bash
uv run penguiflow dev --project-root <project_root> --agent-package company.agent
```

`company.agent` maps to `company/agent` on disk.
...
- Dataset and export paths must remain under project root.
- Loading a directory expects `dataset.jsonl` inside it.
- `max_cases` is capped at 200 and selects rows in file order.
- Runs are synchronous and cases execute sequentially.
- Metric browse lists metrics declared by `evaluate.spec.json` files beneath selected eval root.
