...
- Test: planner with `InMemoryStateStore` that returns `PlannerPause` (e.g., a tool that calls `await ctx.pause("await_input", {...})`) and `tool_context={"session_id": "s1", "trace_id": "t1"}` → after `run()` + loop drain, `store.get_trajectory("t1", "s1")` returns the (paused) trajectory.
...
- Test: `run()` persists buffered events when `run_loop()` raises → use a monkeypatch/stub `run_loop` that emits at least one event then raises; call `planner.run(..., tool_context={\"session_id\": \"s1\", \"trace_id\": \"t1\"})`, catch the exception, drain tasks, then assert `store.list_planner_events("t1")` contains the emitted event(s).
