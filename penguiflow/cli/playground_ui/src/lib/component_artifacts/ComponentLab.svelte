<script lang="ts">
  ...
  $effect(() => {
    if (!selected && sortedComponents.length) {
      selected = sortedComponents[0].name;
    }
  });
  ...
  function useLastArtifact() {
    const last = componentArtifactsStore.lastArtifact;
    if (!last) return;
    ...
    {#if selectedDefinition}
      <header>
        ...
        <div class="actions">
          ...
          <button onclick={useLastArtifact} disabled={!componentArtifactsStore.lastArtifact}>Use Last Artifact</button>
