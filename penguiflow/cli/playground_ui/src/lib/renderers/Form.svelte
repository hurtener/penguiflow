<script lang="ts">
  ...
  type FormValue = string | number | boolean | null | string[] | FileMeta[];
  ...
  function getNumberValidation(field: FormField, key: 'min' | 'max'): number | undefined {
      ...
      >
        ...
        {:else if field.type === 'number'}
          <input
            type="number"
            id={fieldId}
            bind:value={values[field.name] as number | null}
            onblur={() => touched[field.name] = true}
            placeholder={field.placeholder}
            required={field.required}
            disabled={field.disabled}
            min={getNumberValidation(field, 'min')}
            max={getNumberValidation(field, 'max')}
          />
            ...
            {#each normalizeOptions(field.options) as opt}
              <label class="radio-label">
                <input
                  ...
                  name={field.name}
                  value={opt.value}
                  bind:group={values[field.name] as string}
                  disabled={field.disabled}
                />
                <span>{opt.label}</span>
              </label>
        ...
        {:else if field.type === 'range'}
          <input
            type="range"
            id={fieldId}
            bind:value={values[field.name] as number | string}
            min={getNumberValidation(field, 'min') ?? 0}
            max={getNumberValidation(field, 'max') ?? 100}
            disabled={field.disabled}
          />
          <span class="range-value">{values[field.name] as number | string | null}</span>
        ...
        {:else if field.type === 'file'}
          <input
            ...
            disabled={field.disabled}
          />
          {#if Array.isArray(values[field.name]) && (values[field.name] as FileMeta[]).length > 0}
            <div class="file-list">
              {#each (values[field.name] as FileMeta[]) as file}
