...
def render_render_component_guidance(failure_count: int) -> str | None:
    ...
    if failure_count == 1:
        return (
            ...
            "REMINDER: rich-output render tools must produce props that match the target component schema.\n"
    ...
    if failure_count == 2:
        return (
            ...
            "When using `render_component` or any typed `render_*` wrapper, follow this sequence:\n"
            "1) `describe_component(name=...)` to get the exact props schema\n"
            "2) Call the render tool with args that produce props matching that schema exactly\n"
            "If stuck, simplify by rendering `markdown` instead of repeatedly retrying the same invalid payload.\n"
            "</render_component_warning>"
        )
    return (
        "<render_component_critical>\n"
        "CRITICAL: You repeatedly failed to call rich-output render tools with valid props.\n"
        "STOP retrying the same invalid component payload.\n\n"
        "RULES:\n"
        "- If you don't know the schema: call `describe_component(name=...)`.\n"
        "- Then call the wrapper or `render_component` with args that match the schema exactly.\n"
