...
from penguiflow.evals.api import run_eval
...
async def test_run_eval_respects_agent_package_selection(tmp_path: Path) -> None:
    ...
    (good_pkg / "orchestrator.py").write_text(
        ...
        "\n"
        "    async def execute(self, query: str, *, tenant_id: str = 'default'):\n"
        "        return {'answer': f'good:{query}', 'metadata': {'trace_id': f'good-{query}', 'planner': {'steps': [], 'llm_context': {'tenant_id': tenant_id}, 'tool_context': {'tenant_id': tenant_id}}}}\n",
        encoding="utf-8",
    )
    (bad_pkg / "orchestrator.py").write_text(
        ...
        "\n"
        "    async def execute(self, query: str, *, tenant_id: str = 'default'):\n"
        "        return {'answer': f'bad:{query}', 'metadata': {'trace_id': f'bad-{query}', 'planner': {'steps': [], 'llm_context': {'tenant_id': tenant_id}, 'tool_context': {'tenant_id': tenant_id}}}}\n",
        encoding="utf-8",
    )
